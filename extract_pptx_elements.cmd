...
if not defined PYTHON_CMD (
    ...
    echo   本 .cmd 已经内嵌完整源码，无需旁边再放 .py 文件；但 Windows 仍需要 Python 3 解释器。
...
set "SELF=%~f0"
set "PY_TEMP=%TEMP%\extract_pptx_elements_%RANDOM%%RANDOM%.py"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$lines = Get-Content -LiteralPath $env:SELF; $marker = '# ---PYTHON_CODE_BELOW---'; $idx = [Array]::IndexOf($lines, $marker); if ($idx -lt 0) { exit 2 }; $lines[($idx + 1)..($lines.Count - 1)] | Set-Content -LiteralPath $env:PY_TEMP -Encoding UTF8"
if %ERRORLEVEL% neq 0 (
