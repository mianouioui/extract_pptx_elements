...
if [ $# -eq 0 ]; then
    echo "╔══════════════════════════════════════════╗"
    echo "║     PPTX 元素还原器 $VERSION           ║"
    echo "║     restore_pptx_elements               ║"
    echo "╚══════════════════════════════════════════╝"
    echo ""
    echo "把修好的图片写回 PPTX，所有元素位置保持不变。"
    echo ""
    if [ -f "pptx_extracted_elements/manifest.csv" ] || [ -f "manifest.csv" ]; then
        echo "✓ 已在当前文件夹检测到提取素材，开始一键还原..."
        echo ""
    else
        echo "用法: 将“提取文件夹”（含 manifest.csv）拖拽到此窗口，按回车"
        echo "      或把本文件放到该文件夹旁边后直接双击"
        echo ""
        echo "常用选项:"
        echo "  --images-only  只写回图片，其它元素保持原样"
        echo "  --pptx 文件    指定原始 PPTX（默认自动查找）"
        echo "  -o 文件名      指定输出 PPTX 名称"
        echo "  --overwrite    覆盖已存在的输出文件"
        echo "  --dry-run      只预演不生成文件"
        echo ""
        echo "──────────────────────────────────────────"
        read -r -p "请拖入提取文件夹（可留空自动查找）: " input_args
...
PY_TEMP="$(mktemp /tmp/restore_pptx_elements_XXXXXX.py)" || exit 1
